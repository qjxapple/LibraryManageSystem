...
//用户查询租赁窗口
public class UserBookOrderRecordView extends JInternalFrame {
	/**
	 * 
	 */
	...
	public UserBookOrderRecordView() {
	...
	private void init() {
		...
		// combox空间初始化
		cb_type = new JComboBox<String>(new String[] { "查看预约的书籍", "指定书名的预约记录" });
		// 查询类型label初始化
	...
	private void registListener() {
		btn_query.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (index == 1 && content.equals("")) {
					JOptionPane.showInternalMessageDialog(
							UserBookOrderRecordView.this, "查询内容不能为空！");
					return;
				}
				...
				if (index == 0) {
					...
					orderList = orderBiz.queryOrdering();// 查找预约中的记录
				...
				if (orderList.size() == 0) {
					JOptionPane.showInternalMessageDialog(
							UserBookOrderRecordView.this, "没有你要查询的内容！");
				}
		...
		btn_exit.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				int flag = JOptionPane.showInternalConfirmDialog(
						UserBookOrderRecordView.this, "是否确认退出？", "确认信息",
						JOptionPane.YES_NO_CANCEL_OPTION);
				if (flag == JOptionPane.YES_OPTION) {
					UserBookOrderRecordView.this.dispose();
				}
			}
		});
	}
